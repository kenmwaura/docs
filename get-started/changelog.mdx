...
### Version 2.9.4 Changelog - 21st October 2025

#### Enhanced
- Optimized database for faster chat loading speeds
- Enhanced FormSelect component with upward dropdown positioning option to prevent modal boundary clipping
- Enhanced timezone settings with dynamic time format options showing actual time in selected timezone
- Enhanced language system with comprehensive user language management including profile language selection, automatic language persistence across all requests, streamlined menu interface, and seamless language switching for both user and admin interfaces
- Enhanced admin general settings with custom scripts input field for adding JavaScript, CSS, or HTML scripts to the frontend head section

#### Fixed
- Fixed subscription plan deletion to properly handle active subscribers with transfer functionality
- Fixed deleted subscription plans appearing in admin organizations view
- Fixed plan transfer modal dropdown being cut off by modal boundaries
- Fixed logo fallback display when logo is removed from storage - now shows company name instead of broken image
