...
Client ID and Secret are displayed here:
...
Both are displayed under the Sandbox and Live environments. Choose the appropriate environment based on your needs.
...
In the URL field, enter the URL of your webhook endpoint. This should be an endpoint in your application where PayPal will send event notifications (e.g., https://yourapp.com/paypal/webhook).
Under Event types, select the events you want PayPal to notify you about (e.g., PAYMENT.SALE.COMPLETED for successful payments, PAYMENT.SALE.REFUNDED, etc.).
Click Save.
